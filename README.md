<h1 align="center">Hi, I'm Mahalaxmi 👋</h1>
<h3 align="center">AI & DS Undergrad · Building ML systems that ship</h3>

<p align="center">
  <a href="https://linkedin.com/in/mahalaxmi-k-584684319">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:mahalaxmikannan51@gmail.com">
    <img src="https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=MahalaxmiKannan&color=1D9E75&style=flat" />
</p>

---

### About me

- 🎓 B.Tech AI & Data Science @ SKCET, Coimbatore (2024–2028)
- 🤖 I build end-to-end ML systems — from raw data to deployed product
- 🛠 Currently exploring: LLM fine-tuning, agentic AI, real-time ML inference
- 🏆 2x Hackathon Finalist (HackSprint IamNeo · Care Hackathon)
- 📬 Open to internships, collaborations, and interesting problems

---

### Tech stack

**AI / ML**
<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white" />
  <img src="https://img.shields.io/badge/Scikit--learn-F7931E?style=flat&logo=scikit-learn&logoColor=white" />
  <img src="https://img.shields.io/badge/HuggingFace-FFD21E?style=flat&logo=huggingface&logoColor=black" />
  <img src="https://img.shields.io/badge/NLP-Transformers-blue?style=flat" />
</p>

**Full-stack**
<p>
  <img src="https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB" />
  <img src="https://img.shields.io/badge/Django-092E20?style=flat&logo=django&logoColor=white" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white" />
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat&logo=springboot&logoColor=white" />
</p>

**Databases & tools**
<p>
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Power_BI-F2C811?style=flat&logo=powerbi&logoColor=black" />
  <img src="https://img.shields.io/badge/Google_Cloud-4285F4?style=flat&logo=googlecloud&logoColor=white" />
</p>

---

### GitHub stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=MahalaxmiKannan&show_icons=true&theme=default&hide_border=true&count_private=true" height="150" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=MahalaxmiKannan&layout=compact&theme=default&hide_border=true" height="150" />
</p>
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=MahalaxmiKannan&theme=default&hide_border=true" />
</p>

---

### Featured projects

| Project | Stack | Highlight |
|---|---|---|
| 🎙 Speech Recognition Model | Python, TensorFlow | 90%+ accuracy on custom dataset |
| 💼 NexusPay Salary Predictor | Scikit-learn, Python | Predicts salary from skills + experience |
| 📊 Carbon Emission Tracker | React, Django, MongoDB | Converts daily habits into CO₂ insights |
| 🧠 LeetCode Tutor Extension | JavaScript, Browser API | Contextual hints for coding problems |
| 🏨 Resort Booking System | React, Spring Boot | Full-stack reservation management |

---

<p align="center">
  <i>⚡ "I don't just study AI — I build with it."</i>
</p>
